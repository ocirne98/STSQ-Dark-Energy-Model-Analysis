...
class model:
    def __init__(self, z_i, z_c, N, eta, decision):
        ...
        #initial redshift z_i, reiterations N, eta parameter, critical redshift z_c, True/False for DGP model's boolean
        self.z_i = z_i 
        self.N = N
        self.eta = eta
        self.z_c = z_c
    ...
    def CDM(self, a_ax):
        ...
        for k in range(len(a_ax[0:-1])): #o anche range(len(tempoCDM_ax)) 
    ...
    def stsq(self):
        ...
        return array([phi_ax, x_ax, a_ax, z_ax, g_ax, fg_ax, omegam_ax, H_ax,omegade_ax])
    ...
    """
    # check for lum. distance function
    def distance(self, a_ax, H_ax):
        lista1 = a_ax[1:]
        lista2 = a_ax[0:-1]
        lista3 = subtract(lista1,lista2)
        dist_ax = []
        for k in range((len(a_ax))):
            ...
            for j in range(len(lista_da)):
                ...
                H = H_ax[j]
                s += da/((aa**2)*H)
            dist = self.c*self.a_0*s/a
            dist_ax.append(dist/(self.metre*3.086e22))
        return dist_ax
    """
